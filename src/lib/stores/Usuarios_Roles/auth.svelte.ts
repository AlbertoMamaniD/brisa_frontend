// src/stores/auth.svelte.ts - SOLUCIÓN COMPLETA CON LOGS DETALLADOS
...
class AuthStore {
  ...
  async init() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const rolNombre = response.data.roles && response.data.roles.length > 0 
          ? response.data.roles[0].nombre 
          : 'Usuario';
        
        // ✅ Token válido - Actualizar usuario
        this.state.user = {
          usuario_id: response.data.id_usuario || response.data.usuario_id,
        ...
        console.log('✅ Usuario verificado:', {
          ...
          id: this.state.user.usuario_id,
          nombres: this.state.user.nombres,
  ...
  async cargarPermisosDetallados() {
    try {
      console.log('📋 Cargando permisos detallados para:', this.state.user?.usuario);
      ...
      if (response.success && response.data && this.state.user) {
        ...
        console.log('✅ Permisos detallados cargados para:', this.state.user.usuario);
        console.log('   - Usuario ID:', this.state.user.usuario_id);
        console.log('   - Nombres:', this.state.user.nombres);
        console.log('   - Rol:', this.state.user.rol);
        console.log('   - Módulos accesibles:', response.data.modulos_accesibles);
        console.log('   - Acciones disponibles:', response.data.acciones_disponibles?.length || 0);
        console.log('   - Es administrador:', response.data.es_administrador);
      }
    } catch (error) {

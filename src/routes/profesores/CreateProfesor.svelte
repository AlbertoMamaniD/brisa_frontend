<script lang="ts">
    ...
    function agregarAsignacion() {
        if (!puedeGestionarProfesores()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.profesores.asignar}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
    ...
    function removerAsignacion(index: number) {
        if (!puedeGestionarProfesores()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para eliminar asignaciones. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
            ...
            <div class="assignments-column">
                <section class="full-height">
                    ...
                    <p class="section-desc">
                        Asigne cursos y materias (opcional)
                    </p>

                    <div class="assignment-form-stack">
                        <div class="form-group">
                            <label>Curso</label>
                            <select
                    ...
                    <div class="assignments-list-sidebar">
                        {#if asignaciones.length > 0}
                            <div class="list-container-sidebar">
                                {#each asignaciones as asig, i}
                                    <div class="assignment-item-sidebar">
                                        <div class="asig-info">
                                            <span class="materia"
                        ...
                        {:else}
                            <div class="empty-state">
                                <p>Sin asignaciones agregadas</p>
                            </div>
                        {/if}
                    </div>
                </section>
...
<style>
    ...
    .nuevo-profesor {
        ...
        max-width: 1100px; /* MATCH EDIT SIZE */
        margin: 0 auto;
    ...
    section {
        ...
        margin-bottom: 16px; /* Spacing between form sections */
    }

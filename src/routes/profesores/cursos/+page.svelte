<script lang="ts">
    ...
    function abrirNuevo() {
        if (!puedeGestionarCursos()) {
            ...
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.cursos.crear}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
        mostrarNuevo = true;
    ...
    function abrirEditar(c: any) {
        if (!puedeGestionarCursos()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.cursos.editar}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
        cursoSeleccionado = c;
    ...
    function confirmDelete(c: any) {
        if (!puedeGestionarCursos()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.cursos.eliminar}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
        cursoAEliminar = c;

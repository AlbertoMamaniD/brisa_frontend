<script lang="ts">
    ...
    function abrirNuevo() {
        if (!puedeGestionarMaterias()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.materias.crear}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
        mostrarNuevo = true;
    ...
    function abrirEditar(m: Materia) {
        if (!puedeGestionarMaterias()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.materias.editar}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
    ...
    function confirmDelete(m: Materia) {
        if (!puedeGestionarMaterias()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.materias.eliminar}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
        materiaAEliminar = m;
